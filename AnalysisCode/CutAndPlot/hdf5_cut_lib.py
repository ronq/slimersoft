...
class hdf5_cut_lib:
    ...
    def computeCuts(self,df,lower_limit, variableName, upper_limit, cutAction):
	    ...
	    for i in range(len(variableName)):
	        if df.contains(variableName[i]):   # of course we only operate on this DataFrame if it actually contains the variable 
	            ...
	            logicResult=True*len(df)       # if the DataFrame does NOT contain the variable, apply no cut 
	            logicArrayList.append(logicResult)    
